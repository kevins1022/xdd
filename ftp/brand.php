...
if (!$smarty->is_cached('brand.dwt', $cache_id)) {
    ...
    $goodslist = brand_get_goods($brand_id, $cate, $size, $page, $sort, $order,$nlast);
    if ($_GET['act'] == 'asynclist') {
        $sayList = array();
 ...
 * @return  array
 */
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order,$nlast) {
    $cate_where = ($cate > 0) ? 'AND ' . get_children($cate) : '';
    $nlast = isset($nlast)?$nlast:0;
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
            ...
            "ORDER BY $sort $order LIMIT $nlast, 10";
           //echo $sql;
